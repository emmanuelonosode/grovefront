...
const testimonials = [
  ...
  {
    quote:
      "I had bad rental history and no one would work with me. The team here took the time to understand my situation and found a landlord willing to give me a chance.",
    name: "Marcus T.",
    city: "Atlanta, GA",
    image:
...
const specializations = [
  {
    ...
    description:
      "Houses under $1,800/mo across all our markets. We specialize in quality units at honest prices.",
    tag: "Most Popular",
  },
  {
    title: "Family Homes",
    description:
      "3+ bedroom homes near top-rated schools with safe neighbourhoods. We know the school district maps better than most principals.",
    tag: "",
  },
  {
    title: "Corporate Relocation",
    description:
      "Fast-tracked viewings, virtual tours, and guaranteed move-in timelines for employees relocating on company time.",
    tag: "",
  },
  {
    title: "First-Time Renters",
    description:
      "No rental history? We work with first-time renters and help you build the documentation you need for approval.",
    tag: "No Credit OK",
  },
  {
    title: "Section 8 / HCV",
    description:
      "Several of our properties accept Housing Choice Vouchers. Our agents know exactly which listings are voucher-friendly.",
    tag: "",
  },
  {
    title: "Pet-Friendly Homes",
    description:
      "Over 40% of our listings accept pets. We filter your search by pet policy so you only see genuinely pet-welcoming options.",
    tag: "",
  },
];
...
const breadcrumb = {
  ...
  itemListElement: [
    ...
    },
    {
      ...
      position: 2,
      name: "Our Team",
      item: "https://primefamilyhousing.com/agents",
    },
  ],
};
      ...
      <section className="max-w-7xl mx-auto px-6 lg:px-8 py-24 lg:py-32">
        <div className="max-w-xl mb-16">
          ...
          <p className="text-neutral-500 mt-5 text-base leading-relaxed">
            There are thousands of real estate agents. Here&apos;s what makes
            working with ours different.
          </p>
        </div>
          ...
          {agents.length === 0 ? (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              {[
                {
                  name: "Sarah Mitchell",
                  role: "Senior Leasing Specialist",
                ...
                <div
                  key={p.name}
                  className="bg-white border border-neutral-100 rounded-sm overflow-hidden shadow-sm hover:shadow-xl transition-shadow duration-300 group cursor-pointer"
                >
                  ...
                  <div className="p-6">
                    ...
                    <p className="text-brand text-xs font-semibold tracking-wide mt-0.5 mb-4">
                      {p.role}
                    </p>
                    ...
                    <Link
                      href="/contact"
                      className="flex items-center justify-center gap-2 bg-brand-dark text-white text-sm font-medium py-3 rounded-sm hover:bg-brand transition-colors"
                    >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              {agents.map((agent) => (
                <div
                  key={agent.id}
                  className="bg-white border border-neutral-100 rounded-sm overflow-hidden shadow-sm hover:shadow-xl transition-shadow duration-300 group"
                >
                    ...
                    {(agent.agent_profile?.specialties ?? []).length > 0 && (
                      <div className="flex flex-wrap gap-1.5 mb-4">
                        {agent.agent_profile!.specialties.map((s) => (
                          <span
                            key={s}
                            className="text-[10px] tracking-wide bg-brand/8 border border-brand/15 text-brand-dark px-2.5 py-1 rounded-sm"
                          >
                            {s}
                          </span>
                        ))}
      ...
      <section className="max-w-7xl mx-auto px-6 lg:px-8 py-24 lg:py-32">
        <div className="max-w-xl mb-16">
          ...
          <p className="text-neutral-500 mt-5 text-sm leading-relaxed">
            Not every housing search is the same. Our team is trained in the
            specific situations renters and buyers face most often.
          </p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {specializations.map((spec) => (
            <div
              key={spec.title}
              className="group border border-neutral-200 bg-white p-7 rounded-sm hover:border-brand/40 hover:shadow-lg transition-all duration-300 cursor-default"
            >
              <div className="flex items-start justify-between mb-3">
                <h3 className="font-serif text-lg font-bold text-brand-dark group-hover:text-brand transition-colors">
                  {spec.title}
                </h3>
                {spec.tag && (
                  <span className="text-[9px] font-bold tracking-widest uppercase bg-brand/10 text-brand px-2.5 py-1 rounded-sm shrink-0 ml-3">
                    {spec.tag}
                  </span>
                )}
              </div>
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="space-y-3">
            {faqs.map((faq) => (
              <details
                key={faq.q}
                className="group border border-neutral-200 rounded-sm bg-white overflow-hidden hover:border-brand/30 transition-colors"
              >
                <summary className="flex items-center justify-between gap-4 px-6 py-5 cursor-pointer list-none hover:bg-neutral-50 transition-colors">
                  <span className="font-medium text-sm text-brand-dark leading-snug">
                    {faq.q}
                  </span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
          ...
          <div>
            ...
            <p className="text-neutral-600 text-sm leading-relaxed mb-8">
              We&apos;re always looking for motivated specialists who share our
              commitment to affordable housing. If you&apos;re licensed in Utah
              or one of our target markets, we&apos;d love to talk.
            </p>
            <div className="flex flex-col sm:flex-row gap-3">
              <Link
                href="/careers"
                className="inline-flex items-center justify-center gap-2 bg-brand-dark text-white text-sm font-semibold px-7 py-4 rounded-sm hover:bg-brand transition-colors"
              >
                View Careers <ArrowRight size={14} />
              </Link>
              <a
                href="mailto:career@primefamilyhousing.com"
                className="inline-flex items-center justify-center gap-2 border border-brand-dark text-brand-dark text-sm font-medium px-7 py-4 rounded-sm hover:bg-brand-dark hover:text-white transition-colors"
              >
                <Mail size={14} /> career@primefamilyhousing.com
              </a>
            </div>
          </div>
        </div>
